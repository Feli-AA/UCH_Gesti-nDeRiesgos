#!/usr/bin/env bash
# entorno/setup.sh
# Levanta el entorno de SimpleRisk para el TP con Docker Compose.
#
...
echo "==> Levantando SimpleRisk + MySQL + SMTP con Docker Compose..."
docker compose up -d

echo "==> Esperando a que SimpleRisk responda en http://localhost:8080 ..."
tries=0
until curl -fsS -o /dev/null "http://localhost:8080" || [ "$tries" -ge 30 ]; do
  tries=$((tries + 1))
  sleep 5
